...
class Message {
    ...
    replaceTextVariables(text, fields) {
        ...
        vars.forEach(item => {
            const fieldName = fields.find(f => f.field === 'Имя' ? ('NAME' === item.toUpperCase()) : '')// Зарезервировано Имя
            const fieldAdd = fields.find(f => f.field_id ? (`FIELD_${f.field_id}` === item.toUpperCase()) : ''); // Дополнительные поля
            if (fieldName && fieldName.answer) return text = text.replace(`{${item}}`, fieldName.answer);
            if (fieldAdd && fieldAdd.answer) return text = text.replace(`{${item}}`, fieldAdd.answer);
