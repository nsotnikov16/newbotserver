...
class Processing {
    ...
    async createData({ data, history }, { type_do, message, sec }) {
        ...
        switch (type_do) {
            case 'button':
                await this.buttonProcessing();
                break;
            case 'delay':
                Delay.handleCron(this); // обработка ответа длительной задержки с крона
            default:
                await this.defaultProcessing();
                break;
        }

        await this.nextStepProcessing();
    }
        ...
        if (answer.element) {
            ...
            if (this.questionId && answer.parent.id == this.questionId) {
                Api.answerToQuestion({
                    ...
                    field: answer.parent.field.name,
                    field_id: answer.parent.field.id,
                    answer: answer.element.title,
        ...
        if (startBlock && startBlock.question) {
            ...
            if (questionElement) { // Если сообщение пользователя (не кнопка) являлось ответом на вопрос бота
                Api.answerToQuestion({
                    ...
                    field: questionElement.field.name,
                    field_id: questionElement.field.id,
                    answer: this.message,
    ...
    async nextStepProcessing() {
        while (this.nextStep) {
            ...
            const userInfo = await Api.getUserAnswers({ bot_id: this.bot_id, session_id: this.session_id });
            let stepElements = this.nextStep.data.elements;
            switch (this.nextStep.type) {
                case 'message':
                    Message.addDataProcessing(this, stepElements, userInfo);
                    break;
                case 'delay':
                ...
                case 'random':
                    ...
                    if (findRandom && findRandom.nextId) {
                        this.nextStep = this.data.find(el => el.id == findRandom.nextId);
                        continue;
                    }
                    break;
                case 'actions':
